...
export default async function EditQueueBrief({ params }: { params: Promise<{ slug: string }> }) {
    ...
    <main className="app-shell">
      <nav className="app-nav">
        <div className="app-container flex h-16 items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Link href="/fila" className="button-primary px-3 py-2 text-base"><span className="nav-icon" aria-hidden="true">☰</span><span>Fila</span></Link>
      ...
      <div className="app-container page-section">
        <header className="mb-8">
          <RouteHistory items={[{ label: "Dashboard", href: "/", icon: "◐" }, { label: "Fila", href: "/fila", icon: "☰" }, { label: queueBrief.briefId, href: `/fila/${queueBrief.slug}` }, { label: "Edição", icon: "✎" }]} />
          <p className="eyebrow mt-5 mb-2">Edição na fila</p>
          <h1 className="text-4xl font-semibold text-[var(--text-strong)]">Editar brief</h1>
          <p className="mt-3 max-w-3xl text-base leading-6 muted">{queueBrief.headline ?? queueBrief.slug}</p>
        </header>
        <BriefEditForm brief={queueBrief} state="pendente-aprovacao" backHref={`/fila/${queueBrief.slug}`} />
      </div>
    </main>
  );
}
