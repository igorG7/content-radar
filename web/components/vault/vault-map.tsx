...
import { IconAlert, IconCheck, IconLock, IconPencil } from "@/components/ui/icons";
...
function destinoDe(bloco: BlocoMapeado): string {
  if (bloco.trancado && bloco.bloqueador) return `/config/vault/${bloco.bloqueador.key}`;
...
export function VaultMap() {
  ...
  const pctFalta = Math.round((progresso.faltam.length / progresso.total) * 100);
          ...
          <div className="panel-body vault-progresso">
            <div className="row-between">
              ...
              <span className="num small">
                <span className="strong">{progresso.preenchidos}</span> de {progresso.total} blocos
              </span>
            </div>
            <div
              ...
              >
                ...
                <div className="bloco-corpo">
                  <div className="row-tight" style={{ gap: 9 }}>
                    ...
                    <span className={`pill ${PILL[bloco.estado]}`}>{ROTULO[bloco.estado]}</span>
                  </div>
                  <p className="small muted" style={{ marginTop: 5 }}>
                    {bloco.trancado && bloco.bloqueador ? (
                      <>
                        Precisa de <span className="strong">{bloco.bloqueador.titulo}</span> primeiro
                        — esta etapa consome o que sai de lá.
                      </>
                    ) : (
                      bloco.resumo
                    )}
                  ...
                  <div style={{ marginTop: 7 }}>
                    {bloco.preenchido && bloco.atualizado_em ? (
                      ...
                      <span className="meta">degrada sem — a copy sai correta e sem alma</span>
                    ) : (
                      <span className="meta">{CRITICIDADE[bloco.criticidade]}</span>
                    )}
                  </div>
                </div>
      ...
      <div className="stack">
          ...
          <div className="panel-body">
            {progresso.podeRodar ? (
              <div
                className="alert"
                style={{
                  borderColor: "color-mix(in oklch, var(--ok) 40%, transparent)",
              ...
              >
                ...
                <div className="alert-body">
                  ...
                  <p className="small" style={{ marginTop: 3 }}>
                    A próxima varredura usa a versão atual de cada bloco e carimba qual foi.
                  </p>
                </div>
              </div>
            ) : (
              <>
                <div className="alert alert-warning">
                  ...
                  <div className="alert-body">
                    ...
                    <p className="small" style={{ marginTop: 3 }}>
                      Não é queda de qualidade: sem estes blocos não há o que classificar nem onde
                      procurar.
                    </p>
                  </div>
                </div>
                <div className="stack-sm" style={{ marginTop: 14 }}>
                  {progresso.faltam.map((bloco) => (
                    <Link className="row-between" href={destinoDe(bloco)} key={bloco.key}>
                      <span className="small strong">{bloco.titulo}</span>
                      <span className="meta">{bloco.trancado ? "trancado" : "responder"}</span>
                    </Link>
                  ))}
                </div>
              </>
        ...
        <div className="panel">
          ...
          <div className="panel-body stack-sm">
            {[
              ["pill-ok", "preenchido", "Tem versão aceita. Reabrir gera uma nova."],
              [
                "pill-warn",
                "falta para rodar",
              ...
              ["", "trancado", "Consome o que sai de outro bloco. Diz qual e leva até ele."],
        ...
        <div className="panel">
          ...
          <div className="panel-body">
            <p className="small">
              Esta aba é casca: os blocos ficam no navegador, não no store. A persistência real está
              desenhada em <span className="num">docs/design-vault-onboarding.md</span>; hoje o
              vocabulário de verdade vem do vault da empresa em{" "}
