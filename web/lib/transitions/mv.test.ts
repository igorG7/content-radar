...
async function seed(
  frontmatterTail: string,
  mediaFiles = [`${SLUG}__0.jpg`, `${SLUG}__1.jpg`],
) {
  ...
  await writeFile(
    path.join(briefsDir["pendente-aprovacao"], `${SLUG}.md`),
    brief(frontmatterTail),
  );
...
describe("approve", () => {
  ...
  it("moves the brief, keeps only the chosen photo and records the event", async () => {
    const result = await runTransition(
      { slug: SLUG, direction: "approve" },
      paths,
    );

    expect(result.applied).toBe(true);
    expect(await readdir(paths.briefsDir["pendente-aprovacao"])).toEqual([]);
    expect(await readdir(paths.briefsDir["pendente-publicacao"])).toEqual([
      `${SLUG}.md`,
    ]);
  ...
  it("touches only updated_at in the frontmatter", async () => {
    const before = await readFile(
      path.join(paths.briefsDir["pendente-aprovacao"], `${SLUG}.md`),
      "utf8",
    );
    await runTransition({ slug: SLUG, direction: "approve" }, paths);
    const after = await readFile(
      path.join(paths.briefsDir["pendente-publicacao"], `${SLUG}.md`),
      "utf8",
    );
...
describe("regras duras", () => {
  it("refuses to approve when hero_choice is absent", async () => {
    ...
    await expect(
      runTransition({ slug: SLUG, direction: "approve" }, paths),
  ...
  it("refuses a hero_choice with no matching candidate", async () => {
    ...
    await expect(
      planTransition({ slug: SLUG, direction: "approve" }, paths),
  ...
  it("reports an unknown slug", async () => {
    await seed("hero_choice: 0");
    await expect(
      planTransition({ slug: "nao-existe", direction: "approve" }, paths),
    ).rejects.toMatchObject({
      code: "not_found",
    });
  });
...
describe("reject", () => {
  ...
  it("moves to rejeitado, purges all media and appends the reason", async () => {
    ...
    const content = await readFile(
      path.join(paths.briefsDir.rejeitado, `${SLUG}.md`),
      "utf8",
    );
...
describe("dry-run", () => {
  ...
  it("changes nothing on disk", async () => {
    const result = await runTransition(
      { slug: SLUG, direction: "approve", dryRun: true },
      paths,
    );
